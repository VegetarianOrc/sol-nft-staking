...
describe('gmoot-staking', () => {
  ...
  describe('end to end test', async () => {
    ...
    const [rewarder, rewarderBump] = await anchor.web3.PublicKey.findProgramAddress([Buffer.from(collectionName), gmootStakingProgram.programId.toBuffer(), Buffer.from("rewarder")], gmootStakingProgram.programId);
    const [rewardAuthority, rewardAuthorityBump] = await anchor.web3.PublicKey.findProgramAddress([Buffer.from(collectionName), gmootStakingProgram.programId.toBuffer(), Buffer.from("rewarder"), rewarder.toBuffer()], gmootStakingProgram.programId);
    const [stakeAccount, stakeAccountBump] = await anchor.web3.PublicKey.findProgramAddress([Buffer.from(collectionName), gmootStakingProgram.programId.toBuffer(), Buffer.from("stake_account"), rewarder.toBuffer(), owner.publicKey.toBuffer()], gmootStakingProgram.programId);
    const rewardRate = 10;
    let rewardMint = null;
